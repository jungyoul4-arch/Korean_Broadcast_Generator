...
- [A], [B] 등 구간 표시:
  - 원본 이미지에 구분선(세로선/가로선)이 구간 문자와 함께 있는 경우: <div class="section-marker"><span class="section-marker-label">[A]</span></div> 형태로 감싸기
  - 구분선 없이 구간 문자만 있는 경우: 그대로 텍스트로 유지
...
## ★★★ 환각 금지 (절대 규칙) ★★★
- 원본 이미지에 없는 텍스트를 절대 추가하지 마세요
- "다음 글을 읽고 물음에 답하시오" 등 이미지에 없는 지시문을 만들어내지 마세요
- 이미지 하단에 잘려서 일부만 보이는 문장은 포함하지 마세요
- 이미지에 보이는 내용만 정확히 추출하세요. 추측이나 보완은 금지입니다
...
- 밑줄: <u>내용</u> (한 글자짜리도 반드시 감지, 예: ⓛ<u>집</u>)
- ⓐⓑⓒⓓⓔ, ⓛⓜⓝ 등 기호는 유니코드 그대로. 동그라미 기호 뒤 텍스트에 밑줄이 있는지 반드시 확인
- [A], [B] 등 구간 표시:
  - 원본 이미지에 구분선(세로선/가로선)이 구간 문자와 함께 있는 경우: <div class="section-marker"><span class="section-marker-label">[A]</span></div> 형태로 감싸기
  - 구분선 없이 구간 문자만 있는 경우: 그대로 텍스트로 유지
- 한자 병기 유지
